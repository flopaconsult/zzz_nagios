...
Chef::Log.info("---------- Founded Hosts: #{nodes}")
if nodes.empty?
  Chef::Log.info("No nodes returned from search, using this node so hosts.cfg has data")
  nodes = Array.new
end

# first chef-run will not find monitoring machine in the above search
myself_included = 0;
nodes.each do |n|
...
if myself_included == 0
  nodes << node
end
Chef::Log.info("---------- All Hosts: #{nodes}")
...
search(:role, "*:*") do |r|
  role_list << r.name
  search(:node, "roles:#{r.name} AND app_environment:#{node[:app_environment]}") do |n|
    service_hosts[r.name] = n['hostname']
    Chef::Log.info("role: #{r.name}  host: #{n['hostname']}")
  end
end

Chef::Log.info("---------- Hostgroups: #{role_list}")
Chef::Log.info("---------- Services hosts/hostgroups: #{service_hosts}")
